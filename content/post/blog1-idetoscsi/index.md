...
---
## DISCLAIMER!!
This is not tech advice, this is not a tutorial. But if you are a beginner perhaps this could aid you in your desperate searches for the exact problem you are encountering. And if you are not then maybe my blundering could provide entertainment.
...
After monitoring my VM performance, I noticed consistently high CPU usage and poor responsiveness despite allocating more resources. So, I did some research and found that IDE is one of the slowest virtual disk controllers/interfaces you can use in proxmox. This is because there is much more overhead when emulating this kind of legacy hardware technology. I believe this was causing my high CPU usage and slow speeds. Safe to say, converting my virtual storage to SCSI would at least help alleviate this slow performance I was seeing. Or it would elimintate this hypothesis and I could focus somewhere else.

For some reason I assumed the drivers would be preinstalled so I simply detached my hard drive, changed it to SCSI and then attached it again. Nope. I forgot nothing can be that simple in virtualization. So I finally searched up the documentation, read through it and felt confident it would be a breeze.
